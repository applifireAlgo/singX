Ext.define('Buzzor.singx.web.singx.controller.singx.WireTransferModeMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.WireTransferModeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#WireTransferModeTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#WireTransferMode');
          var grid = this.view.down('#WireTransferModeGrid');
          var tree = this.view.down('#WireTransferModeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#WireTransferModeGrid');
               ...
               var formPanel = this.getView().up().down("#WireTransferMode");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#WireTransferModeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#WireTransferMode').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#WireTransferModeGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.wireTransferModeId = grid.store.data.items[rowIndex].data.wireTransferModeId;
          me = this;
          Ext.MessageBox.confirm({
               title: 'Confirm',
               msg: 'Delete Feature',
     ...
     loadData: function(responseData) {
          this.view.down('#WireTransferMode').getViewModel().setData(responseData.response.data);
     },
