Ext.define('Buzzor.singx.web.singx.controller.singx.SingxCityMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.SingxCityMainController',
     ...
     stateIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#stateId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'stateId', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#SingxCityTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#SingxCity');
          var grid = this.view.down('#SingxCityGrid');
          var tree = this.view.down('#SingxCityTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#SingxCityGrid');
               ...
               var formPanel = this.getView().up().down("#SingxCity");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#SingxCityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SingxCity').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SingxCityGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.cityId = grid.store.data.items[rowIndex].data.cityId;
          me = this;
          Ext.MessageBox.confirm({
               title: 'Confirm',
               msg: 'Delete Feature',
     ...
     loadData: function(responseData) {
          this.view.down('#SingxCity').getViewModel().setData(responseData.response.data);
     },
