Ext.define('Buzzor.singx.web.singx.controller.singx.CfgDocCorporateMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CfgDocCorporateMainController',
     ...
     companyTypeLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#companyType');
                    scope.sender.controller.assignAllComboData(scope.sender, 'companyType', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onCountryIdChange: function(me, newValue, oldValue, eOpts) {
          this.onCountryIdChangeCompanyCategory(me, newValue, oldValue, eOpts);
     },
     onCountryIdChangeCompanyCategory: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               ...
               element: me,
               success: function(response, scope) {
                    var companyCategoryCombo = scope.element.up('form').down('#companyCategory');
                    scope.sender.controller.assignComboData(companyCategoryCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, companyCategoryCombo);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CfgDocCorporateTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CfgDocCorporate');
          var grid = this.view.down('#CfgDocCorporateGrid');
          var tree = this.view.down('#CfgDocCorporateTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CfgDocCorporateGrid');
               ...
               var formPanel = this.getView().up().down("#CfgDocCorporate");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CfgDocCorporateTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CfgDocCorporate').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CfgDocCorporateGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.documentId = grid.store.data.items[rowIndex].data.documentId;
          me = this;
          Ext.MessageBox.confirm({
               title: 'Confirm',
               msg: 'Delete Feature',
     ...
     loadData: function(responseData) {
          this.view.down('#CfgDocCorporate').getViewModel().setData(responseData.response.data);
     },
