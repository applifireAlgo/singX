Ext.define('Buzzor.singx.web.singx.controller.singx.CfgCountryAddressMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CfgCountryAddressMainController',
     ...
     inputTypeIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/InputDataType/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#inputTypeId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'inputTypeId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CfgCountryAddressTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CfgCountryAddress');
          var grid = this.view.down('#CfgCountryAddressGrid');
          var tree = this.view.down('#CfgCountryAddressTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CfgCountryAddressGrid');
               ...
               var formPanel = this.getView().up().down("#CfgCountryAddress");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CfgCountryAddressTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CfgCountryAddress').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CfgCountryAddressGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#CfgCountryAddress').getViewModel().setData(responseData.response.data);
     },
