Ext.define('Buzzor.singx.web.singx.controller.singx.CorporateRegDetailMainController', {
     extend: 'Buzzor.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CorporateRegDetailMainController',
     init: function() {
          ...
          this.companyTypeIdLoad();
          this.customerIdLoad();
          this.companyCategoryIdLoad();
     },
     companyTypeIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/CompanyType/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#companyTypeId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'companyTypeId', response.responseText);
     ...
     customerIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/SingXCustomer/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#customerId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'customerId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#customerId');
     ...
     companyCategoryIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/CompanyCategory/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#companyCategoryId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'companyCategoryId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#companyCategoryId');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CorporateRegDetailTree');
               ...
               var compRef = this.view.down('#CorporateRegDetailTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CorporateRegDetail');
          var grid = this.view.down('#CorporateRegDetailGrid');
          var tree = this.view.down('#CorporateRegDetailTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CorporateRegDetailTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var CorporateRegDetail = this.view.down('#CorporateRegDetailForm');
               CorporateRegDetail.reset();
               var gridPanel = this.view.down('#CorporateRegDetailGrid');
          ...
          var formPanel = this.getView().up().down('#CorporateRegDetailForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CorporateRegDetailTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CorporateRegDetail').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CorporateRegDetailGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#CorporateRegDetailForm').getViewModel().setData(responseData.response.data);
     },
