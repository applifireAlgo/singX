Ext.define('Buzzor.singx.web.singx.controller.singx.SingXCustomerMainController', {
     extend: 'Buzzor.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.SingXCustomerMainController',
     init: function() {
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#SingXCustomerTree');
               ...
               var compRef = this.view.down('#SingXCustomerTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.customerId = grid.store.data.items[rowIndex].data.customerId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#SingXCustomer');
          var grid = this.view.down('#SingXCustomerGrid');
          var tree = this.view.down('#SingXCustomerTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#SingXCustomerTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var SingXCustomer = this.view.down('#SingXCustomerForm');
               SingXCustomer.reset();
               var gridPanel = this.view.down('#SingXCustomerGrid');
          ...
          var formPanel = this.getView().up().down('#SingXCustomerForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#SingXCustomerTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SingXCustomer').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SingXCustomerGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#SingXCustomerForm').getViewModel().setData(responseData.response.data);
     },
