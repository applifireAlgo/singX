Ext.define('Buzzor.singx.web.singx.controller.singx.BusinessDocumentMainController', {
     extend: 'Buzzor.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.BusinessDocumentMainController',
     init: function() {
     ...
     documentIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/CfgDocCorporate/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#BusinessDocumentTree');
               ...
               var compRef = this.view.down('#BusinessDocumentTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#BusinessDocument');
          var grid = this.view.down('#BusinessDocumentGrid');
          var tree = this.view.down('#BusinessDocumentTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#BusinessDocumentTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var BusinessDocument = this.view.down('#BusinessDocumentForm');
               BusinessDocument.reset();
               var gridPanel = this.view.down('#BusinessDocumentGrid');
          ...
          var formPanel = this.getView().up().down('#BusinessDocumentForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#BusinessDocumentTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#BusinessDocument').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#BusinessDocumentGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#BusinessDocumentForm').getViewModel().setData(responseData.response.data);
     },
