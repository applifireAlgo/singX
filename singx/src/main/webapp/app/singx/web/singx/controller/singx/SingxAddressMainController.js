Ext.define('Buzzor.singx.web.singx.controller.singx.SingxAddressMainController', {
     extend: 'Buzzor.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.SingxAddressMainController',
     init: function() {
     ...
     stateLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#state');
                    scope.sender.controller.assignAllComboData(scope.sender, 'state', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#state');
     ...
     districtLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#district');
                    scope.sender.controller.assignAllComboData(scope.sender, 'district', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#district');
     ...
     countryLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#country');
                    scope.sender.controller.assignAllComboData(scope.sender, 'country', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#country');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#SingxAddressTree');
               ...
               var compRef = this.view.down('#SingxAddressTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.addressId = grid.store.data.items[rowIndex].data.addressId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#SingxAddress');
          var grid = this.view.down('#SingxAddressGrid');
          var tree = this.view.down('#SingxAddressTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#SingxAddressTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var SingxAddress = this.view.down('#SingxAddressForm');
               SingxAddress.reset();
               var gridPanel = this.view.down('#SingxAddressGrid');
          ...
          var formPanel = this.getView().up().down('#SingxAddressForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#SingxAddressTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SingxAddress').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SingxAddressGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#SingxAddressForm').getViewModel().setData(responseData.response.data);
     },
