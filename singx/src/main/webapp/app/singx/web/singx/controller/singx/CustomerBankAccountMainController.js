Ext.define('Buzzor.singx.web.singx.controller.singx.CustomerBankAccountMainController', {
     extend: 'Buzzor.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CustomerBankAccountMainController',
     init: function() {
          ...
          this.contactIdLoad();
          this.wireTransferModeIdLoad();
          this.bankIdLoad();
     },
     contactIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/SingxContacts/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
     ...
     wireTransferModeIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/WireTransferMode/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#wireTransferModeId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'wireTransferModeId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#wireTransferModeId');
     ...
     bankIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/Bank/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#bankId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'bankId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#bankId');
     ...
     onBankIdChange: function(me, newValue, oldValue, eOpts) {
          this.onBankIdChangeBranchId(me, newValue, oldValue, eOpts);
     },
     onBankIdChangeBranchId: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/BankBranch/findByBankId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var branchIdCombo = scope.element.up('form').down('#branchId');
                    scope.sender.controller.assignComboData(branchIdCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, branchIdCombo);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CustomerBankAccountTree');
               ...
               var compRef = this.view.down('#CustomerBankAccountTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.mapId = grid.store.data.items[rowIndex].data.mapId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CustomerBankAccount');
          var grid = this.view.down('#CustomerBankAccountGrid');
          var tree = this.view.down('#CustomerBankAccountTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CustomerBankAccountTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var CustomerBankAccount = this.view.down('#CustomerBankAccountForm');
               CustomerBankAccount.reset();
               var gridPanel = this.view.down('#CustomerBankAccountGrid');
          ...
          var formPanel = this.getView().up().down('#CustomerBankAccountForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CustomerBankAccountTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CustomerBankAccount').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CustomerBankAccountGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#CustomerBankAccountForm').getViewModel().setData(responseData.response.data);
     },
