Ext.define('Buzzor.singx.web.singx.controller.singx.SingxContactsMainController', {
     extend: 'Buzzor.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.SingxContactsMainController',
     init: function() {
     ...
     onCountryIdChange: function(me, newValue, oldValue, eOpts) {
          this.onCountryIdChangePrefixId(me, newValue, oldValue, eOpts);
          this.onCountryIdChangePhoneCountryCode(me, newValue, oldValue, eOpts);
     },
     onCountryIdChangePrefixId: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/SingxPrefix/findByCountryId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var prefixIdCombo = scope.element.up('form').down('#prefixId');
                    scope.sender.controller.assignComboData(prefixIdCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, prefixIdCombo);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onCountryIdChangePhoneCountryCode: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/CallingCode/findByCountryId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var phoneCountryCodeCombo = scope.element.up('form').down('#phoneCountryCode');
                    scope.sender.controller.assignComboData(phoneCountryCodeCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, phoneCountryCodeCombo);
               },
               failure: function(response, scope) {
                    debugger;
                    scope.sender.controller.addCommunicationLog(response, false);
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#SingxContactsTree');
               ...
               var compRef = this.view.down('#SingxContactsTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.contactId = grid.store.data.items[rowIndex].data.contactId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#SingxContacts');
          var grid = this.view.down('#SingxContactsGrid');
          var tree = this.view.down('#SingxContactsTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#SingxContactsTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var SingxContacts = this.view.down('#SingxContactsForm');
               SingxContacts.reset();
               var gridPanel = this.view.down('#SingxContactsGrid');
          ...
          var formPanel = this.getView().up().down('#SingxContactsForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#SingxContactsTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SingxContacts').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SingxContactsGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#SingxContactsForm').getViewModel().setData(responseData.response.data);
     },
