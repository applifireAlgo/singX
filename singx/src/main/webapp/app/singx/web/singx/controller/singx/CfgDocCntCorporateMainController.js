Ext.define('Buzzor.singx.web.singx.controller.singx.CfgDocCntCorporateMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CfgDocCntCorporateMainController',
     ...
     onCountryIdChange: function(me, newValue, oldValue, eOpts) {
          this.onCountryIdChangeCompanyCategoryId(me, newValue, oldValue, eOpts);
     },
     onCountryIdChangeCompanyCategoryId: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/CompanyCategory/findByCountryId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var companyCategoryIdCombo = scope.element.up('form').down('#companyCategoryId');
                    scope.sender.controller.assignComboData(companyCategoryIdCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, companyCategoryIdCombo);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CfgDocCntCorporateTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CfgDocCntCorporate');
          var grid = this.view.down('#CfgDocCntCorporateGrid');
          var tree = this.view.down('#CfgDocCntCorporateTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CfgDocCntCorporateGrid');
               ...
               var formPanel = this.getView().up().down("#CfgDocCntCorporate");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CfgDocCntCorporateTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CfgDocCntCorporate').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CfgDocCntCorporateGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.cfgDocId = grid.store.data.items[rowIndex].data.cfgDocId;
          me = this;
          Ext.MessageBox.confirm({
               title: 'Confirm',
               msg: 'Delete Feature',
     ...
     loadData: function(responseData) {
          this.view.down('#CfgDocCntCorporate').getViewModel().setData(responseData.response.data);
     },
