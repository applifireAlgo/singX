Ext.define('Buzzor.singx.web.singx.controller.singx.CfgDocumentMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CfgDocumentMainController',
     ...
     customerTypeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/CustomerType/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#customerType');
                    scope.sender.controller.assignAllComboData(scope.sender, 'customerType', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CfgDocumentTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CfgDocument');
          var grid = this.view.down('#CfgDocumentGrid');
          var tree = this.view.down('#CfgDocumentTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CfgDocumentGrid');
               ...
               var formPanel = this.getView().up().down("#CfgDocument");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CfgDocumentTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CfgDocument').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CfgDocumentGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#CfgDocument').getViewModel().setData(responseData.response.data);
     },
