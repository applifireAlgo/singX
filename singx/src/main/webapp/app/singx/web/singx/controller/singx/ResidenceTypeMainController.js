Ext.define('Buzzor.singx.web.singx.controller.singx.ResidenceTypeMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ResidenceTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ResidenceTypeTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#ResidenceType');
          var grid = this.view.down('#ResidenceTypeGrid');
          var tree = this.view.down('#ResidenceTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#ResidenceTypeGrid');
               ...
               var formPanel = this.getView().up().down("#ResidenceType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#ResidenceTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ResidenceType').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ResidenceTypeGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.residencTypeId = grid.store.data.items[rowIndex].data.residencTypeId;
          me = this;
          Ext.MessageBox.confirm({
               title: 'Confirm',
               msg: 'Delete Feature',
     ...
     loadData: function(responseData) {
          this.view.down('#ResidenceType').getViewModel().setData(responseData.response.data);
     },
