Ext.define('Buzzor.singx.web.singx.controller.singx.SingxPrefixMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.SingxPrefixMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#SingxPrefixTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#SingxPrefix');
          var grid = this.view.down('#SingxPrefixGrid');
          var tree = this.view.down('#SingxPrefixTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#SingxPrefixGrid');
               ...
               var formPanel = this.getView().up().down("#SingxPrefix");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#SingxPrefixTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SingxPrefix').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SingxPrefixGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.prefixId = grid.store.data.items[rowIndex].data.prefixId;
          me = this;
          Ext.MessageBox.confirm({
               title: 'Confirm',
               msg: 'Delete Feature',
     ...
     loadData: function(responseData) {
          this.view.down('#SingxPrefix').getViewModel().setData(responseData.response.data);
     },
