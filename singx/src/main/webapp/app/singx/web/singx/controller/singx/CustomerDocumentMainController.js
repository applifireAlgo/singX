Ext.define('Buzzor.singx.web.singx.controller.singx.CustomerDocumentMainController', {
     extend: 'Buzzor.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CustomerDocumentMainController',
     init: function() {
     ...
     documentIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/CfgDocIndividual/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#documentId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'documentId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#documentId');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CustomerDocumentTree');
               ...
               var compRef = this.view.down('#CustomerDocumentTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CustomerDocument');
          var grid = this.view.down('#CustomerDocumentGrid');
          var tree = this.view.down('#CustomerDocumentTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CustomerDocumentTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var CustomerDocument = this.view.down('#CustomerDocumentForm');
               CustomerDocument.reset();
               var gridPanel = this.view.down('#CustomerDocumentGrid');
          ...
          var formPanel = this.getView().up().down('#CustomerDocumentForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CustomerDocumentTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CustomerDocument').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CustomerDocumentGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#CustomerDocumentForm').getViewModel().setData(responseData.response.data);
     },
