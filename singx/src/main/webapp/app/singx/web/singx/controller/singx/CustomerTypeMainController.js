Ext.define('Buzzor.singx.web.singx.controller.singx.CustomerTypeMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CustomerTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CustomerTypeTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CustomerType');
          var grid = this.view.down('#CustomerTypeGrid');
          var tree = this.view.down('#CustomerTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CustomerTypeGrid');
               ...
               var formPanel = this.getView().up().down("#CustomerType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CustomerTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CustomerType').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CustomerTypeGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.customerTypeId = grid.store.data.items[rowIndex].data.customerTypeId;
          me = this;
          Ext.MessageBox.confirm({
               title: 'Confirm',
               msg: 'Delete Feature',
     ...
     loadData: function(responseData) {
          this.view.down('#CustomerType').getViewModel().setData(responseData.response.data);
     },
