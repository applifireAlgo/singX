Ext.define('Buzzor.singx.web.singx.controller.singx.BankBranchMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.BankBranchMainController',
     ...
     onCountryIdChange: function(me, newValue, oldValue, eOpts) {
          this.onCountryIdChangeBankId(me, newValue, oldValue, eOpts);
     },
     onCountryIdChangeBankId: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/Bank/findByCountryId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var bankIdCombo = scope.element.up('form').down('#bankId');
                    scope.sender.controller.assignComboData(bankIdCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, bankIdCombo);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#BankBranchTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#BankBranch');
          var grid = this.view.down('#BankBranchGrid');
          var tree = this.view.down('#BankBranchTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#BankBranchGrid');
               ...
               var formPanel = this.getView().up().down("#BankBranch");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#BankBranchTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#BankBranch').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#BankBranchGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.branchId = grid.store.data.items[rowIndex].data.branchId;
          me = this;
          Ext.MessageBox.confirm({
               title: 'Confirm',
               msg: 'Delete Feature',
     ...
     loadData: function(responseData) {
          this.view.down('#BankBranch').getViewModel().setData(responseData.response.data);
     },
