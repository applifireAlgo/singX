Ext.define('Buzzor.singx.web.singx.controller.singx.AgentMainController', {
     extend: 'Buzzor.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.AgentMainController',
     init: function() {
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#AgentTree');
               ...
               var compRef = this.view.down('#AgentTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.agentId = grid.store.data.items[rowIndex].data.agentId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Agent');
          var grid = this.view.down('#AgentGrid');
          var tree = this.view.down('#AgentTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#AgentTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var Agent = this.view.down('#AgentForm');
               Agent.reset();
               var gridPanel = this.view.down('#AgentGrid');
          ...
          var formPanel = this.getView().up().down('#AgentForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#AgentTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Agent').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AgentGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#AgentForm').getViewModel().setData(responseData.response.data);
     },
     ...
     saveForm: function(but, evt) {
          ...
          var CoreContactsForm = form.down('#CoreContactsForm');
          if (!CoreContactsForm.isValid()) {
               this.showCard(form, CoreContactsForm);
          }
