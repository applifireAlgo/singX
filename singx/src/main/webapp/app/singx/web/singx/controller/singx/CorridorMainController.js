Ext.define('Buzzor.singx.web.singx.controller.singx.CorridorMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CorridorMainController',
     ...
     toCountryIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#toCountryId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'toCountryId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CorridorTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Corridor');
          var grid = this.view.down('#CorridorGrid');
          var tree = this.view.down('#CorridorTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CorridorGrid');
               ...
               var formPanel = this.getView().up().down("#Corridor");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CorridorTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Corridor').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CorridorGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var jsonData = {};
          jsonData.corridorId = grid.store.data.items[rowIndex].data.corridorId;
          me = this;
          Ext.MessageBox.confirm({
               title: 'Confirm',
               msg: 'Delete Feature',
     ...
     loadData: function(responseData) {
          this.view.down('#Corridor').getViewModel().setData(responseData.response.data);
     },
